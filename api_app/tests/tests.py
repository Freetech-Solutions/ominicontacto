...
class APITest(OMLBaseTest):
    ...
    def test_api_vista_pausa_de_agente_retorno_de_valores_correctos(self, pause_agent, manager):
        ...
        cant_eventos = AgentActivityEventV2.objects.count()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['status'], 'OK')
        self.assertEqual(AgentActivityEventV2.objects.count(), cant_eventos + 1)
        evento = AgentActivityEventV2.objects.order_by('-id').first()
        self.assertEqual(evento.agente_id, self.agente_profile.id)
        self.assertEqual(evento.event_type, AgentActivityEventV2.EventType.STATE_PAUSED)
    ...
    def test_api_vista_pausa_de_agente_retorno_de_valores_erroneos(self, pause_agent, manager):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['status'], 'ERROR')
        self.assertEqual(AgentActivityEventV2.objects.count(), cant_eventos)
    ...
    def test_api_vista_despausa_de_agente_retorno_de_valores_correctos(
        ...
        evento = AgentActivityEventV2.objects.order_by('-id').first()
        self.assertEqual(evento.agente_id, self.agente_profile.id)
        self.assertEqual(evento.event_type, AgentActivityEventV2.EventType.STATE_READY)
