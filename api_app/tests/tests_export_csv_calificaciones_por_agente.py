...
class ExportCSVCalificacionesPorAgenteAPITest(OMLBaseTest):
    def setUp(self):
        super(ExportCSVCalificacionesPorAgenteAPITest, self).setUp()
        admin = self.crear_administrador(username='admin_export_calificaciones_agente')
        self.client.login(username=admin.username, password=PASSWORD)
        self.url = reverse('api_exportar_csv_calificaciones_por_agente')
    ...
    def test_post_valido_devuelve_ok_y_arranca_thread(self, mock_thread, _mock_permission):
        ...
        self.assertEqual(
            thread_args[0],
            'OML:STATUS_CSV_REPORT:DISPOSITIONS_BY_AGENT:{0}:{1}'.format(
                self.campana.pk,
                task_id,
            ),
        )
