...
class VoicebotWebhookView(APIView):
    ...
    def _construir_observaciones(body_data):
        ...
        Reglas:
          ...
          - Excluye en raíz 'call_id' y 'callid' (duplicados de X-OML-Call-ID).
          - Excluye en cualquier nivel: contactID, customerID, phone, CampaignID.
        ...
        EXCLUDED_KEYS = {'contactID', 'customerID', 'phone', 'CampaignID'}
    ...
    def _resolver_agente(request, campana):
        ...
        Selecciona el agente a asociar a la calificación, en este orden:
          ...
          2. settings.VOICEBOT_BOT_AGENT_USERNAME → agente con ese username.
        ...
        bot_username = getattr(settings, 'VOICEBOT_BOT_AGENT_USERNAME', None)
        if bot_username:
            ...
            if agente:
                logger.info(
                    'Voicebot: usando agente BOT configurado "%s" (id=%s)',
                    bot_username, agente.id,
                )
                return agente
            logger.warning(
                'Voicebot: VOICEBOT_BOT_AGENT_USERNAME="%s" no corresponde a un agente activo',
                bot_username,
            )
        ...
        if agentes_campana.exists():
            ...
            logger.info(
                'Voicebot: usando primer agente de la campaña %s (id=%s)',
        ...
        if agente:
            logger.warning(
                'Voicebot: sin agentes en la campaña %s, usando primer agente activo (id=%s)',
                campana.id, agente.id,
            )
            return agente
    ...
    def _publicar_voicebot_transfer_proceed(call_id):
        ...
        try:
            ...
            logger.info(
                'Voicebot: comando voicebot_transfer_proceed publicado en %s para call_id=%s',
                ACD_GLOBAL_COMMANDS_CHANNEL, call_id,
            )
        except Exception:
            logger.exception(
                'Voicebot: error publicando voicebot_transfer_proceed para call_id=%s',
                call_id,
            )
