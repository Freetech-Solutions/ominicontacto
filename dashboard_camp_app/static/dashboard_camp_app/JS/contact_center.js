function registerContactCenterDashboard() {
    Alpine.data('contactCenterDashboard', () => ({
        ...
        data: {
            ...
            inbound_detalle: null,
            outbound_detalle: null,
        },
        loading: false,
        lastUpdate: null,
        initialized: false,
        ...
        async fetchData() {
            this.loading = true;
            try {
                const url = window.CONTACT_CENTER_DATA_URL || '/dashboard/camp/contact-center/data/';
                ...
                // Cargar datos detallados de inbound y outbound si hay campaña seleccionada
                if (this.campaignId) {
                    this.fetchInboundDetalle();
                    this.fetchOutboundDetalle();
                }
        ...
        async fetchAgentes() {
            try {
                const url = window.CONTACT_CENTER_AGENTES_URL || '/dashboard/camp/contact-center/data/agentes/';
                const params = new URLSearchParams();
                if (this.campaignId) {
                    params.set('campaign_id', this.campaignId);
                }
        ...
        async fetchAgentesLista() {
            try {
                const url = window.CONTACT_CENTER_AGENTES_LISTA_URL || '/dashboard/camp/contact-center/data/agentes-lista/';
                const params = new URLSearchParams();
                if (this.campaignId) {
                    params.set('campaign_id', this.campaignId);
                }
        ...
        /**
         * Obtener métricas detalladas de inbound
         */
        async fetchInboundDetalle() {
            if (!this.campaignId) {
                if (this.data.inbound_detalle !== null) {
                    this.data.inbound_detalle = null;
                    this.previousData.inbound_detalle = null;
                }
                return;
            }

            try {
                const url = window.CONTACT_CENTER_INBOUND_DETALLE_URL || '/dashboard/camp/contact-center/data/inbound-detalle/';
                const params = new URLSearchParams();
                params.set('campaign_id', this.campaignId);
                ...
                // Crear nuevo objeto con los datos recibidos
                const newInboundDetalle = {
                    nombre_camp: jsonData.nombre_camp || '',
                    ag_ready: jsonData.ag_ready || 0,
                    ag_oncall: jsonData.ag_oncall || 0,
                    ag_pause: jsonData.ag_pause || 0,
                    llamadas_ofrecidas: jsonData.llamadas_ofrecidas || 0,
                    llamadas_atendidas: jsonData.llamadas_atendidas || 0,
                    llamadas_timeout: jsonData.llamadas_timeout || 0,
                    llamadas_abandonadas: jsonData.llamadas_abandonadas || 0,
                    tiempo_prom_abandono: jsonData.tiempo_prom_abandono || 0,
                    tiempo_prom_espera: jsonData.tiempo_prom_espera || 0,
                    gestiones_positivas: jsonData.gestiones_positivas || 0,
                };

                // Solo actualizar si los datos han cambiado para evitar recursión infinita
                if (this.hasChanged('inbound_detalle', newInboundDetalle, this.previousData.inbound_detalle)) {
                    this.data.inbound_detalle = newInboundDetalle;
                    this.previousData.inbound_detalle = JSON.parse(JSON.stringify(newInboundDetalle));
                }

            } catch (error) {
                console.error('Error obteniendo métricas detalladas de inbound:', error);
                // En caso de error, mantener datos anteriores o establecer null
                if (!this.data.inbound_detalle) {
                    this.data.inbound_detalle = null;
                }
            }
        },
        ...
        /**
         * Obtener métricas detalladas de outbound
         */
        async fetchOutboundDetalle() {
            if (!this.campaignId) {
                if (this.data.outbound_detalle !== null) {
                    this.data.outbound_detalle = null;
                    this.previousData.outbound_detalle = null;
                }
                return;
            }

            try {
                const url = window.CONTACT_CENTER_OUTBOUND_DETALLE_URL || '/dashboard/camp/contact-center/data/outbound-detalle/';
                const params = new URLSearchParams();
                params.set('campaign_id', this.campaignId);
                const fullUrl = `${url}?${params.toString()}`;
                ...
                // Crear nuevo objeto con los datos recibidos
                const newOutboundDetalle = {
                    nombre_camp: jsonData.nombre_camp || (this.campaignId ? `Campaña ${this.campaignId}` : ''),
                    ag_ready: jsonData.ag_ready || 0,
                    ag_oncall: jsonData.ag_oncall || 0,
                    ag_pause: jsonData.ag_pause || 0,
                    att: jsonData.att || 0,
                    discadas: jsonData.discadas || 0,
                    contactadas: jsonData.contactadas || 0,
                    contestadores: jsonData.contestadores || 0,
                    busy: jsonData.busy || 0,
                    congestion: jsonData.congestion || 0,
                    otro_error: jsonData.otro_error || 0,
                    tiempo_prom_abandono: jsonData.tiempo_prom_abandono || 0,
                    tiempo_prom_espera: jsonData.tiempo_prom_espera || 0,
                    gestiones_positivas: jsonData.gestiones_positivas || 0,
                };
                ...
                // Solo actualizar si los datos han cambiado para evitar recursión infinita
                if (this.hasChanged('outbound_detalle', newOutboundDetalle, this.previousData.outbound_detalle)) {
                    this.data.outbound_detalle = newOutboundDetalle;
                    this.previousData.outbound_detalle = JSON.parse(JSON.stringify(newOutboundDetalle));
                }

            } catch (error) {
                console.error('Error obteniendo métricas detalladas de outbound:', error);
                // En caso de error, establecer valores por defecto solo si no existen datos previos
                if (!this.data.outbound_detalle) {
                    this.data.outbound_detalle = {
                        nombre_camp: this.campaignId ? `Campaña ${this.campaignId}` : '',
                        ag_ready: 0,
                        ag_oncall: 0,
                        ag_pause: 0,
                        att: 0,
        ...
        async fetchLlamadas() {
            try {
                const url = window.CONTACT_CENTER_LLAMADAS_URL || '/dashboard/camp/contact-center/data/llamadas/';
                const params = new URLSearchParams();
                if (this.campaignId) {
                    params.set('campaign_id', this.campaignId);
                }
        ...
        onCampaignChange() {
            ...
            this.fetchData();
            // También cargar datos detallados de inbound y outbound
            this.fetchInboundDetalle();
            this.fetchOutboundDetalle();
        },
