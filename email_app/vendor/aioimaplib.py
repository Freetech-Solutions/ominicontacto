...
log = logging.getLogger("aioimaplib")
...
class IMAP4ClientProtocol(asyncio.Protocol):
    ...
    async def starttls(self, host, ssl_context=None):
        if 'STARTTLS' not in self.capabilities:
            Abort('server does not have STARTTLS capability')
        if hasattr(self, '_tls_established') and self._tls_established:
            Abort('TLS session already established')
        response = await self.execute(Command('STARTTLS', self.new_tag(), loop=self.loop))
        if response.result != 'OK':
            return response
