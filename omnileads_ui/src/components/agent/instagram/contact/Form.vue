...
export default {
    ...
    computed: {
        ...mapState(['agtInstagramContactDBFields', 'agtInstagramConversationInfo'])
    },
    methods: {
        ...mapActions([
            'agtInstagramContactCreateFromConversation',
            'agtInstagramContactCreate',
            'agtInstagramContactUpdate'
        ]),
        initializeData () {
            this.initFormData();
        ...
        closeModal () {
            ...
            const event = new CustomEvent('onInstagramContactFormEvent', {
                detail: {
                    contact_form: false
                }
            });
        ...
        initFormData () {
            if (this.agtInstagramContactDBFields?.length > 0) {
                this.form = {};
                for (let i = 0; i < this.agtInstagramContactDBFields.length; i++) {
                    const field = this.agtInstagramContactDBFields[i];
                    let value = null;
                    if (i === 0) {
        ...
        async save () {
            try {
              ...
              !this.isPhoneValid(field?.value)
                ...
                const formData = {
                    campaignId: this.agtInstagramConversationInfo.campaignId,
                    conversationId: this.agtInstagramConversationInfo.id,
                    data: this.getFormData(),
                    contactId: null
                };
                if (this.formToCreate && this.formToCreate != null) {
                    response = await this.agtInstagramContactCreateFromConversation(formData);
                } else if (this.formToCreateFromNewConversation != null && this.formToCreateFromNewConversation) {
                    const formData = {
                        campaignId: localStorage.getItem('agtInstagramCampaingId'),
                        fdata: this.getFormData()
                    };
                    console.log('formData >>', formData);
                    response = await this.agtInstagramContactCreate(formData);
                } else {
                    formData.contactId = this.form.id.value;
                    response = await this.agtInstagramContactUpdate(formData);
                }
    ...
    watch: {
        ...
        agtInstagramConversationInfo: {
            handler () {
                console.log('agtInstagramConversationInfo >>2', this.agtInstagramConversationInfo);
                if (this.agtInstagramConversationInfo.client.id !== null) {
