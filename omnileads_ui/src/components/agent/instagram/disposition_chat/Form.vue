...
import { FORM_TYPES, FIELD_TYPES, FIELD_TYPE_NUMBER } from '@/globals/agent/instagram/disposition';
...
export default {
    ...
    computed: {
        ...mapState([
            ...
            'agtInstagramDispositionChatDetail'
        ])
    },
    methods: {
        ...mapActions([
            'agtInstagramDispositionChatUpdate',
            'agtInstagramDispositionChatCreate'
        ]),
        resetData () {
            this.form = {
                id: null,
        ...
        initFormData () {
            if (this.agtInstagramDispositionChatFormToCreate) {
        ...
        getFormFieldsByOption () {
            ...
            const option = this.agtInstagramDispositionChatOptions?.find(
        ...
        async save (isFormValid) {
            ...
            try {
                ...
                const formData = {
                    idContact: this.agtInstagramConversationInfo?.client?.id || null,
                    idAgente: this.agtInstagramConversationInfo?.agent || null,
                    ...
                    idConversation: this.agtInstagramConversationInfo.id
                };
                // Si es de GESTION hay que mandar respuestaFormularioGestion
                const dispositionType = this.agtInstagramDispositionChatOptions?.find(
                ...
                if (this.agtInstagramDispositionChatFormToCreate) {
                    response = await this.agtInstagramDispositionChatCreate({
                        data: formData
                    });
                } else {
                    response = await this.agtInstagramDispositionChatUpdate({
                        id: this.agtInstagramDispositionChatDetail.id,
                        data: formData
                    });
                }
                this.$swal.close();
                ...
                if (status === HTTP_STATUS.SUCCESS) {
                    const event = new Event(INSTAGRAM_LOCALSTORAGE_EVENTS.CONVERSATION.DETAIL_INIT_DATA);
