...
export default {
    ...
    computed: {
        ...mapState([
            'agtInstagramTransferChatAgents',
            'agtInstagramTransferChatForm',
            'agtInstagramConversationInfo'
        ])
    },
    methods: {
        ...mapActions(['agtInstagramTransferChatSend']),
        closeModal () {
            this.clearData();
            const event = new CustomEvent('onInstagramTransferChatEvent', {
                detail: {
                    transfer_chat: false
                }
            });
        ...
        initFormData () {
            this.form.to = this.agtInstagramTransferChatForm?.to || null;
        },
        clearFilter () {
            this.initFilters();
        },
        ...
        async transfer (isFormValid) {
            try {
                ...
                const conversationId = this.agtInstagramConversationInfo?.id || JSON.parse(
                    localStorage.getItem('agtInstagramConversationInfo')
                )?.id;
                const to = this.form?.to || null;
                const { status, message } = await this.agtInstagramTransferChatSend({
                    to,
                    conversationId
                });
                this.closeModal();
                if (status === HTTP_STATUS.SUCCESS) {
                    const event = new CustomEvent(INSTAGRAM_LOCALSTORAGE_EVENTS.TRANSFER.DONE, {
                        detail: {
                            to,
                            conversationId
                        }
    ...
    watch: {
        agtInstagramTransferChatAgents: {
            handler () {
                if (this.agtInstagramTransferChatAgents) {
                    this.agents = this.agtInstagramTransferChatAgents;
