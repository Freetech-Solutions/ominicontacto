...
export default {
    data () {
        return {
            instagram_color: COLORS.INSTAGRAM.Pink,
            isExpired: false
        };
    },
    props: {
    ...
    methods: {
        ...mapActions(['agtInstagramCoversationRequest']),
        ...
        async requestConversation () {
            const { status, message, data } = await this.agtInstagramCoversationRequest(
            ...
            if (status === HTTP_STATUS.SUCCESS) {
                ...
                localStorage.setItem(
                    'agtInstagramConversationAttending',
                    this.conversationInfo.id
                );
                this.$router.push({
