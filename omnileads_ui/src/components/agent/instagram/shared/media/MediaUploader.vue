...
export default {
    ...
    methods: {
        ...mapActions(['agtInstagramConversationSendAttachmentMessage']),
        getFileType () {
        ...
        async customUploader ($event) {
            try {
                ...
                const agtInstagramConversationInfo = JSON.parse(
                    localStorage.getItem('agtInstagramConversationInfo')
                );
                const file = this.files[0];
                const formData = new FormData();
                formData.append('file', file);
                console.log('>>>>>> agtInstagramConversationInfo:', agtInstagramConversationInfo);
                var data = {
                    conversationId: agtInstagramConversationInfo.id,
                    formData: formData,
                    pageId: agtInstagramConversationInfo.page.page_id,
                    messages: messages,
                    $t: this.$t
                };
                const { status, message } = await this.agtInstagramConversationSendAttachmentMessage(data);
