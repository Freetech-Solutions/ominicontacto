...
export default {
    ...
    computed: {
        ...mapState(['supInstagramAccountTemplates'])
    },
    methods: {
        clearFilter () {
            this.initFilters();
        ...
        async remove (id) {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    ...
                    const { status, message } = await this.deleteInstagramAccountTemplate(
                        id
                    );
                    this.$swal.close();
                    if (status === HTTP_STATUS.SUCCESS) {
                        this.initInstagramAccountTemplates();
                        this.$swal(
        ...
        ...mapActions([
            'deleteInstagramAccountTemplate',
            'initInstagramAccountTemplates',
            'initInstagramAccountTemplate'
        ])
    },
    watch: {
        supInstagramAccountTemplates: {
            handler () {
                this.templates = this.supInstagramAccountTemplates.map((template) => {
                    return {
                        id: template.id,
