...
export default {
    ...
    computed: {
        ...mapState(['supInstagramReportGeneralColors', 'supInstagramReportGeneral'])
    },
    watch: {
        supInstagramReportGeneralColors: {
            handler () {
                this.colors.rgbColors = this.supInstagramReportGeneralColors.rgbColors;
                this.colors.rgbaColors = this.supInstagramReportGeneralColors.rgbaColors;
            },
            deep: true,
            immediate: true
        },
        supInstagramReportGeneral: {
            handler () {
                this.data = [];
                for (const key in this.supInstagramReportGeneral) {
                    this.data.push(this.supInstagramReportGeneral[key]);
                }
            },
            deep: true,
            immediate: true
