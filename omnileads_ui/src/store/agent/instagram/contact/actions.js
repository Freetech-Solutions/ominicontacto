...
import Service from '@/services/agent/instagram/contact_service';
import { HTTP_STATUS } from '@/globals';
const service = new Service();
export default {
    async agtInstagramContactListInit ({ commit }, { campaignId = null, conversationId = null }) {
        try {
            if (!campaignId) {
                await commit('agtInstagramContactListInit', []);
                return {
    ...
    async agtInstagramContactCreateFromConversation (
        { commit },
    ...
    async agtInstagramContactCreate (
        { commit },
        ...
        try {
            ...
            const { status, data } = response;
            await commit('agtInstagramNewContact', status === HTTP_STATUS.SUCCESS ? data : []);
            return response;
        } catch (error) {
            console.error('===> ERROR to create contact');
            console.error(error);
    ...
    async agtInstagramContactUpdate (
        { commit },
    ...
    async agtInstagramContactDBFieldsInit (
        { commit },
        { campaignId = null }
    ) {
        console.log('agtInstagramContactDBFieldsInit action', campaignId);
        try {
            if (!campaignId) {
                await commit('agtInstagramContactDBFieldsInit', []);
                return {
                    status: HTTP_STATUS.ERROR,
            ...
            const { status, data } = response;
            await commit('agtInstagramContactDBFieldsInit', status === HTTP_STATUS.SUCCESS ? data : []);
            return response;
        } catch (error) {
    ...
    async agtInstagramContactSearch (
        { commit },
        ...
        try {
            ...
            const { status, data } = response;
            await commit('agtInstagramContactSearchInit', status === HTTP_STATUS.SUCCESS ? data : []);
            return response;
        } catch (error) {
            console.error('===> ERROR al buscar los contactos');
            console.error(error?.message);
            await commit('agtInstagramContactSearchInit', []);
            return {
                status: HTTP_STATUS.ERROR,
                message: 'Error al buscar contactos'
            };
    ...
    async agtInstagramContactSuggestMatch (
        { commit },
    ...
    async agtInstagramContactAssignToConversation (
        { commit },
