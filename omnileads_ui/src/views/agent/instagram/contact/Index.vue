...
<script>
import Header from '@/components/agent/instagram/contact/Header';
import Form from '@/components/agent/instagram/contact/Form';
import SearchTable from '@/components/agent/instagram/contact/SearchTable';
...
export default {
    ...
    methods: {
        ...mapActions([
            'agtInstagramContactDBFieldsInit',
            'agtInstagramSetConversationInfo',
            'agtInstagramContactAssignToConversation'
        ]),
        cleanFilterSearch () {
        ...
        async selectPreviewContact (contact) {
            ...
            const { status, message } =
                await this.agtInstagramContactAssignToConversation({
        ...
        async updatedLocalStorage () {
            const agtInstagramConversationInfo = localStorage.getItem('agtInstagramConversationInfo');
            if (agtInstagramConversationInfo && agtInstagramConversationInfo !== 'null') {
                this.conversationInfo = JSON.parse(agtInstagramConversationInfo);
            ...
            this.inconmingConversation = localStorage.getItem('agtInstagramInconmingConversation') === 'true';
        }
    },
    mounted () {
