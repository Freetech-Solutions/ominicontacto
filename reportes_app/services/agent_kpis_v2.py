...
def get_agent_activity_kpis_v2(date_start, date_end, agent_id=None, group_by='agent'):
    ...
    # Tabla reportes_app_agentactivityeventv2: ts, agente_id, event_type, pause_id, aux_code,
    # metadata
    sql_query = """
    WITH raw_events AS (
        SELECT
    ...
    intervals AS (
        SELECT
            ...
            EXTRACT(
                EPOCH FROM (
                    COALESCE(next_ts, LEAST(NOW(), %(until)s)) - timestamp
                )
            ) as duration_seconds,
    ...
    SELECT
        ...
        SUM(
            CASE
                WHEN i.event_type IN (
                    'SESSION_LOGIN', 'STATE_READY', 'STATE_PAUSED', 'STATE_ACW'
        ...
        SUM(
            CASE
                WHEN i.event_type = 'STATE_READY'
                    OR i.event_type = 'SESSION_LOGIN'
                THEN i.duration_seconds
                ELSE 0
            END
        ) as ready_seconds,
        SUM(
            CASE
                WHEN i.event_type = 'STATE_PAUSED' THEN i.duration_seconds
                ELSE 0
            END
        ) as pause_seconds,
    ...
    # aux_code: 'UNKNOWN' solo si pause_id IS NULL AND aux_code IS NULL; si pause_id existe =>
    # aux_code NULL o valor real.
    # seconds: int (bigint) para que SUM(pause_breakdown.seconds) sea consistente con pause_seconds
    # (enteros).
    breakdown_subquery = (
        "(SELECT jsonb_agg(jsonb_build_object("
        "'pause_id', pb.pause_id, "
        "'aux_code', CASE WHEN pb.pause_id IS NOT NULL THEN pb.aux_code ELSE COALESCE(pb.aux_code, 'UNKNOWN') END, "  # noqa: E501
    ...
    with connection.cursor() as cursor:
        final_sql = sql_query.replace('%(group_field)s', group_field).replace(
            '__BREAKDOWN_SUBQUERY__', breakdown_subquery)
        cursor.execute(final_sql, params)
        rows = cursor.fetchall()

    # Formateo de respuesta: row = agent_id, group_key, session_seconds, ready, pause, acw,
    # sessions_count, pauses_count, pause_breakdown_json
    results = []
    for row in rows:
        ...
        if pause_breakdown_raw is not None and pause_breakdown_raw != '':
            try:
                breakdown_list = json.loads(pause_breakdown_raw) if isinstance(
                    pause_breakdown_raw, str) else list(pause_breakdown_raw) if pause_breakdown_raw else []  # noqa: E501
        ...
        if pause_seconds > 0 and not breakdown_list:
            logger.warning(
                "agents_kpis_v2: pause_seconds=%s but pause_breakdown empty for agent_id=%s, date=%s; using UNKNOWN entry",  # noqa: E501
