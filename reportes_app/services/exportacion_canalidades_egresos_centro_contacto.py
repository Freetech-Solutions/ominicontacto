...
"""
Servicio para exportar la tabla "Canalidades por campaña" (Egresos) del reporte
...
PREFIJO_ARCHIVO = 'canalidades_egresos_por_campana'
...
def generar_csv_canalidades_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Canalidades por campaña (Egresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:CANALIDADES_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    from api_app.views.reports_centro_contacto import obtener_canalidades_por_campana
    ...
    try:
        ...
        logger.warning("Redis publish(0) canalidades egresos CC: %s", e)

    try:
        rows, totals = obtener_canalidades_por_campana(
        ...
        logger.exception("Error obteniendo canalidades egresos por campaña: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) canalidades egresos CC: %s", e)


def obtener_url_descarga_canalidades_egresos(task_id):
    """
