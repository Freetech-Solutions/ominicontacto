...
"""
Servicio para exportar la tabla "Canalidades por día" del reporte
...
PREFIJO_ARCHIVO = 'canalidades_por_dia'
...
def generar_csv_canalidades_por_dia_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Canalidades por día (Ingresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:CANALIDADES_POR_DIA_CC:cc:{task_id}.  # noqa: E501
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) canalidades por día CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo canalidades por día: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) canalidades por día CC: %s", e)


def obtener_url_descarga_canalidades_por_dia(task_id):
    """
