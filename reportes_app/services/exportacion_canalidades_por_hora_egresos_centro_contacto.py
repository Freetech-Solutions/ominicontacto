...
"""
Servicio para exportar la tabla "Canalidades por hora" (Egresos) del reporte
...
PREFIJO_ARCHIVO = 'canalidades_por_hora_egresos'
...
def generar_csv_canalidades_por_hora_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Canalidades por hora (Egresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:CANALIDADES_POR_HORA_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) canalidades por hora egresos CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo canalidades por hora egresos: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) canalidades por hora egresos CC: %s", e)


def obtener_url_descarga_canalidades_por_hora_egresos(task_id):
    """
