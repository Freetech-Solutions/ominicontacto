...
"""
Servicio para exportar la tabla "Canalidades por mes" del reporte
...
PREFIJO_ARCHIVO = 'canalidades_por_mes'
...
def generar_csv_canalidades_por_mes_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Canalidades por mes (Ingresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:CANALIDADES_POR_MES_CC:cc:{task_id}.  # noqa: E501
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) canalidades por mes CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo canalidades por mes: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) canalidades por mes CC: %s", e)


def obtener_url_descarga_canalidades_por_mes(task_id):
    """
