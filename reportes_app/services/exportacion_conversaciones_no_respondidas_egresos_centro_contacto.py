...
"""
Servicio para exportar la tabla "Conversaciones no respondidas" (Egresos > WhatsApp)
...
PREFIJO_ARCHIVO = 'conversaciones_no_respondidas_egresos'
...
def generar_csv_conversaciones_no_respondidas_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Conversaciones no respondidas (Egresos WhatsApp) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:CONV_NO_RESP_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) conversaciones no respondidas egresos CC: %s", e)

    try:
        qs = ConversacionWhatsapp.objects.conversaciones_salientes_no_atendidas(
            start_date, end_date
        )
        ...
        logger.exception("Error obteniendo conversaciones no respondidas egresos: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) conversaciones no respondidas egresos CC: %s", e)


def obtener_url_descarga_conversaciones_no_respondidas_egresos(task_id):
    """
