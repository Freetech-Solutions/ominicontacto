...
"""
Servicio para exportar la tabla "Conversaciones Respondidas" (Ingresos > WhatsApp)
...
PREFIJO_ARCHIVO = 'conversaciones_respondidas'
...
def generar_csv_conversaciones_respondidas_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Conversaciones Respondidas (Ingresos WhatsApp) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:CONV_RESP_CC:cc:{task_id}.
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) conversaciones respondidas CC: %s", e)

    try:
        qs = ConversacionWhatsapp.objects.conversaciones_entrantes_atendidas(
            start_date, end_date
        )
        ...
        logger.exception("Error obteniendo conversaciones respondidas: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) conversaciones respondidas CC: %s", e)


def obtener_url_descarga_conversaciones_respondidas(task_id):
    """
