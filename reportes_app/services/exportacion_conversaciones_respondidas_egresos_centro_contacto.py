...
"""
Servicio para exportar la tabla "Conversaciones Respondidas" (Egresos > WhatsApp)
...
PREFIJO_ARCHIVO = 'conversaciones_respondidas_egresos'
...
def generar_csv_conversaciones_respondidas_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    try:
        ...
        logger.warning("Redis publish(0) conversaciones respondidas egresos CC: %s", e)

    try:
        qs = ConversacionWhatsapp.objects.conversaciones_salientes_atendidas(
            start_date, end_date
        )
        ...
        logger.exception("Error obteniendo conversaciones respondidas egresos: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) conversaciones respondidas egresos CC: %s", e)


def obtener_url_descarga_conversaciones_respondidas_egresos(task_id):
    """
