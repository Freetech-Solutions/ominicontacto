...
"""
Servicio para exportar la tabla "Listado de llamadas atendidas" (Ingresos/Voz)
...
PREFIJO_ARCHIVO = 'llamadas_atendidas'
...
def generar_csv_llamadas_atendidas_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV del Listado de llamadas atendidas (Ingresos/Voz) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:LLAMADAS_ATENDIDAS_CC:cc:{task_id}.  # noqa: E501
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas atendidas CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo listado llamadas atendidas: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas atendidas CC: %s", e)


def obtener_url_descarga_llamadas_atendidas(task_id):
    """
