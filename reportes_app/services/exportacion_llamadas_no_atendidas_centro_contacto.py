...
"""
Servicio para exportar la tabla "Listado de llamadas no atendidas" (Ingresos/Voz)
...
PREFIJO_ARCHIVO = 'llamadas_no_atendidas'
...
def generar_csv_llamadas_no_atendidas_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas no atendidas CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo listado llamadas no atendidas: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas no atendidas CC: %s", e)


def obtener_url_descarga_llamadas_no_atendidas(task_id):
    """
