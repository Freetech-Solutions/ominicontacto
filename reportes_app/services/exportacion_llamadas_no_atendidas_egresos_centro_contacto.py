...
"""
Servicio para exportar la tabla "Listado de llamadas no atendidas" (Egresos/Voz)
...
PREFIJO_ARCHIVO = 'llamadas_no_atendidas_egresos'
...
def generar_csv_llamadas_no_atendidas_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV del Listado de llamadas no atendidas (Egresos/Voz) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:LLAMADAS_NO_ATENDIDAS_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas no atendidas egresos CC: %s", e)
    ...
    try:
        while True:
            page_obj = obtener_listado_llamadas_no_atendidas(
        ...
        logger.exception("Error obteniendo listado llamadas no atendidas egresos: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas no atendidas egresos CC: %s", e)


def obtener_url_descarga_llamadas_no_atendidas_egresos(task_id):
    """
