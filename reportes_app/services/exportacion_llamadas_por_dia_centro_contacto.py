...
"""
Servicio para exportar la tabla "Llamadas por día" (Ingresos/Voz/Días) del reporte
...
PREFIJO_ARCHIVO = 'llamadas_por_dia'
...
def generar_csv_llamadas_por_dia_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Llamadas por día (Ingresos/Voz/Días) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:LLAMADAS_POR_DIA_CC:cc:{task_id}.  # noqa: E501
    """
    from api_app.views.reports_centro_contacto import obtener_llamadas_por_dia
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas por día CC: %s", e)

    try:
        rows = obtener_llamadas_por_dia(
        ...
        logger.exception("Error obteniendo llamadas por día: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas por día CC: %s", e)


def obtener_url_descarga_llamadas_por_dia(task_id):
    """
