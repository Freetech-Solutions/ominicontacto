...
"""
Servicio para exportar la tabla "Llamadas por hora de día" (Egresos/Voz/Horas) del reporte
...
PREFIJO_ARCHIVO = 'llamadas_por_hora_egresos'
...
def _dist_row_csv(row, label_key='hour_label'):
    return [
        _to_str(row.get(label_key)),
...
def generar_csv_llamadas_por_hora_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Llamadas por hora de día (Egresos/Voz/Horas) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:LLAMADAS_POR_HORA_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    from api_app.views.reports_centro_contacto import obtener_llamadas_salientes_por_hora
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas por hora egresos CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo llamadas salientes por hora: %s", e)
        try:
    ...
    # Aplicar mismo filtro de rango horario que la vista (Egresos/Voz/Horas)
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas por hora egresos CC: %s", e)


def obtener_url_descarga_llamadas_por_hora_egresos(task_id):
    """
