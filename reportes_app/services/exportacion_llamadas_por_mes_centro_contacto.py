...
"""
Servicio para exportar la tabla "Llamadas por mes" (Ingresos/Voz/Mes) del reporte
...
PREFIJO_ARCHIVO = 'llamadas_por_mes'
...
def generar_csv_llamadas_por_mes_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas por mes CC: %s", e)

    try:
        rows = obtener_llamadas_por_mes(
        ...
        logger.exception("Error obteniendo llamadas por mes: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas por mes CC: %s", e)


def obtener_url_descarga_llamadas_por_mes(task_id):
    """
