...
"""
Servicio para exportar la tabla "Llamadas por mes" (Egresos/Voz/Mes) del reporte
...
PREFIJO_ARCHIVO = 'llamadas_por_mes_egresos'
...
def generar_csv_llamadas_por_mes_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Llamadas por mes (Egresos/Voz/Mes) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:LLAMADAS_POR_MES_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    from api_app.views.reports_centro_contacto import obtener_llamadas_salientes_por_mes
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas por mes egresos CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo llamadas salientes por mes: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas por mes egresos CC: %s", e)


def obtener_url_descarga_llamadas_por_mes_egresos(task_id):
    """
