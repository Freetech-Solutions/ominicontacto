...
"""
Servicio para exportar la tabla "Llamadas de voz por campaña" (Egresos/Voz) del reporte
...
PREFIJO_ARCHIVO = 'llamadas_voz_egresos_por_campana'
...
def generar_csv_llamadas_voz_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Llamadas de voz por campaña (Egresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:LLAMADAS_VOZ_EGRESOS_CC:cc:{task_id}.
    """
    from api_app.views.reports_centro_contacto import obtener_llamadas_salientes_por_campana
    ...
    try:
        ...
        logger.warning("Redis publish(0) llamadas voz egresos CC: %s", e)
    ...
    try:
        ...
        logger.exception("Error obteniendo llamadas de voz egresos por campaña: %s", e)
        try:
    ...
    if rows:
        ...
        pct_conectadas = (100.0 * total_conectadas / total_sent) if total_sent else 0.0
        pct_no_conectadas = (100.0 * (total_sent - total_conectadas) / total_sent) if total_sent else 0.0
        totals = {
            'sent': total_sent,
    ...
    try:
        ...
        logger.warning("Redis publish(100) llamadas voz egresos CC: %s", e)


def obtener_url_descarga_llamadas_voz_egresos(task_id):
    """
