...
"""
Servicio para exportar la tabla "Mensajes por campaña" (Ingresos > WhatsApp > Campañas)
...
PREFIJO_ARCHIVO = 'whatsapp_mensajes_por_campana'
...
def generar_csv_whatsapp_mensajes_por_campana(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    try:
        ...
        logger.warning("Redis publish(0) whatsapp mensajes por campaña: %s", e)

    try:
        rows = obtener_whatsapp_mensajes_por_campana(
        ...
        logger.exception("Error obteniendo whatsapp mensajes por campaña: %s", e)
        try:
    ...
    with open(ruta, 'w', newline='', encoding='utf-8') as f:
        ...
        count_duracion = 0
        for row in rows:
            recibidos = row.get('recibidos') or 0
            ...
            if row.get('avg_frt_segundos') is not None:
                sum_frt += float(row['avg_frt_segundos']) * recibidos
                count_frt += recibidos
    ...
    try:
        ...
        logger.warning("Redis publish(100) whatsapp mensajes por campaña: %s", e)


def obtener_url_descarga_whatsapp_mensajes_por_campana(task_id):
    """
