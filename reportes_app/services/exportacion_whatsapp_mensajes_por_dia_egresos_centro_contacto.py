...
"""
Servicio para exportar la tabla "Mensajes por día" (Egresos > WhatsApp > Días)
...
PREFIJO_ARCHIVO = 'whatsapp_mensajes_por_dia_egresos'
...
def generar_csv_whatsapp_mensajes_por_dia_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Mensajes por día (WhatsApp Egresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:WA_MSG_DIA_EGRESOS_CC:cc:{task_id}.  # noqa: E501
    """
    from api_app.views.reports_centro_contacto import obtener_whatsapp_egresos_mensajes_por_dia
    ...
    try:
        ...
        logger.warning("Redis publish(0) whatsapp mensajes por dia egresos: %s", e)

    try:
        rows, totals = obtener_whatsapp_egresos_mensajes_por_dia(
        ...
        logger.exception("Error obteniendo whatsapp mensajes por dia egresos: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) whatsapp mensajes por dia egresos: %s", e)


def obtener_url_descarga_whatsapp_mensajes_por_dia_egresos(task_id):
    """
