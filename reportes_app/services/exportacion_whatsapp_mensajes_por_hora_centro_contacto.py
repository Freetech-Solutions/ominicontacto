...
"""
Servicio para exportar la tabla "Mensajes por hora de día" (Ingresos > WhatsApp > Horas)
...
PREFIJO_ARCHIVO = 'whatsapp_mensajes_por_hora'
KEY_TASK_TEMPLATE = 'OML:STATUS_CSV_REPORT:WA_MSG_HORA:cc:{task_id}'
...
def generar_csv_whatsapp_mensajes_por_hora(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Mensajes por hora de día (WhatsApp Ingresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:WA_MSG_HORA:cc:{task_id}.
    """
    from api_app.views.reports_centro_contacto import obtener_whatsapp_mensajes_por_hora
    ...
    try:
        ...
        logger.warning("Redis publish(0) whatsapp mensajes por hora: %s", e)

    try:
        rows, totals = obtener_whatsapp_mensajes_por_hora(
        ...
        logger.exception("Error obteniendo whatsapp mensajes por hora: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) whatsapp mensajes por hora: %s", e)


def obtener_url_descarga_whatsapp_mensajes_por_hora(task_id):
    """
