...
"""
Servicio para exportar la tabla "Mensajes por mes" (Ingresos > WhatsApp > Campañas > Mes)
...
PREFIJO_ARCHIVO = 'whatsapp_mensajes_por_mes'
...
def generar_csv_whatsapp_mensajes_por_mes(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    """
    Genera el archivo CSV de Mensajes por mes (WhatsApp Ingresos) y publica
    progreso en Redis (0 y 100). key_task debe ser OML:STATUS_CSV_REPORT:WA_MSG_MES:cc:{task_id}.
    """
    from api_app.views.reports_centro_contacto import obtener_whatsapp_mensajes_por_mes
    ...
    try:
        ...
        logger.warning("Redis publish(0) whatsapp mensajes por mes: %s", e)

    try:
        rows, totals = obtener_whatsapp_mensajes_por_mes(
        ...
        logger.exception("Error obteniendo whatsapp mensajes por mes: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) whatsapp mensajes por mes: %s", e)


def obtener_url_descarga_whatsapp_mensajes_por_mes(task_id):
    """
