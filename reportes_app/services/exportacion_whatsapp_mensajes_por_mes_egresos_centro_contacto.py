...
"""
Servicio para exportar la tabla "Mensajes por mes" (Egresos > WhatsApp > Mes)
...
PREFIJO_ARCHIVO = 'whatsapp_mensajes_por_mes_egresos'
...
def generar_csv_whatsapp_mensajes_por_mes_egresos_centro_contacto(
    key_task,
    task_id,
    start_date=None,
    end_date=None,
    ...
    try:
        ...
        logger.warning("Redis publish(0) whatsapp mensajes por mes egresos: %s", e)

    try:
        rows, totals = obtener_whatsapp_egresos_mensajes_por_mes(
        ...
        logger.exception("Error obteniendo whatsapp mensajes por mes egresos: %s", e)
        try:
    ...
    try:
        ...
        logger.warning("Redis publish(100) whatsapp mensajes por mes egresos: %s", e)


def obtener_url_descarga_whatsapp_mensajes_por_mes_egresos(task_id):
    """
